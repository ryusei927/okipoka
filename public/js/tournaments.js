// js/tournaments.js (PC/スマホ表示切替最終版)
...
function getTournamentTimeHtml(start, end, now, timeLabel, hasRealLateReg) {
    ...
    if (now < start) {
        const totalMinutes = Math.round((start - now) / (1000 * 60));
        if (totalMinutes <= 60) {
            const formattedTime = formatMinutes(totalMinutes);
            return `<div class="tournament-time">${timeLabel}</div>
                    <div class="status-starting-soon">開始まで ${formattedTime}</div>`;
        }
    }
    if (now >= start && now <= end) {
        let liveStatusContent = `<span class="status-in-progress">開催中</span>`;
        if (hasRealLateReg) {
            const totalMinutes = Math.round((end - now) / (1000 * 60));
            const formattedTime = formatMinutes(totalMinutes);
            liveStatusContent += `<span class="status-countdown"> Late ${formattedTime}</span>`;
        }
        return `<div class="tournament-time">${timeLabel}</div>
                <div class="live-status-wrapper">${liveStatusContent}</div>`;
    }
    return `<div class="tournament-time">${timeLabel}</div>`;
}
...
function updateAllCardStatuses() {
    ...
    document.querySelectorAll('.tournament-card').forEach(card => {
        ...
        const end = hasRealLateReg ? new Date(lateRegStr) : new Date(start.getTime() + 6 * 60 * 60 * 1000);
...
function createTournamentCard(doc, selectedDate, logoMap) {
    ...
    if (startDateTime && lateRegDateTime && lateRegDateTime < startDateTime) { lateRegDateTime.setDate(lateRegDateTime.getDate() + 1); }
    const hasRealLateReg = !!lateRegDateTime;
    const endDateTime = hasRealLateReg ? lateRegDateTime : new Date(startDateTime.getTime() + 6 * 60 * 60 * 1000);
    ...
    return `
      <a href="/tournaments/${doc.id}" class="tournament-card" 
        ...
        <div class="tournament-col-time" data-status-container>${timeDisplayHtml}</div>
        <div class="tournament-col-details">
            <div class="tournament-title">${data.eventName || '名称未設定'}</div>
            <div class="tournament-store">${logoImg} <span>${data.storeName || ''}</span></div>
        </div>
...
function injectStyles() {
    const fullStyles = `
      ...
      .status-starting-soon { font-size: 0.9rem; font-weight: bold; color: #17a2b8; margin-top: 4px; }
      .live-status-wrapper { margin-top: 4px; }

      /* PC(デフォルト)では縦に並ぶように設定 */
      .status-in-progress, .status-countdown { display: block; }
      .status-in-progress { font-size: 0.9rem; font-weight: bold; color: #28a745; }
      .status-countdown { font-size: 0.9rem; font-weight: bold; color: #d9534f; margin-top: 4px; }
      ...
      @media (max-width: 768px) {
        ...
        .status-starting-soon { font-size: 0.8rem; }
        
        /* スマホでは横に並ぶように設定を上書き */
        .status-in-progress, .status-countdown { display: inline; font-size: 0.8rem; }
        .status-countdown { margin-top: 0; margin-left: 0.5em; }
      }`;
