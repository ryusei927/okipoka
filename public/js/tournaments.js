...
async function loadTournaments(dateStr) {
    ...
    const sortedDocs = snapshot.docs.sort((a, b) => {
        const timeA = a.data().startTime || '00:00';
        const timeB = b.data().startTime || '00:00';
        return timeA.localeCompare(timeB);
    });

    list.innerHTML = sortedDocs.map(doc => createTournamentCard(doc)).join('');
}

function createTournamentCard(doc) {
    const data = doc.data();
    ...
    return `
      <a href="/tournaments/${doc.id}" class="tournament-card">
        <div class="tournament-col-time">
          <div class="tournament-time">${data.startTime || ''}</div>
        ...
        <div class="tournament-col-details">
          ...
          <div class="tournament-store">${data.storeName || ''}</div>
        </div>
