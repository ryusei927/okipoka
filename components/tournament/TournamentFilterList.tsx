  ...
  return (
    <div>
      ...
      <div className="max-w-md md:max-w-none mx-auto px-4 py-3 flex items-center gap-2">
        {/* カレンダー風日付表示 */}
        <div className="shrink-0 w-11 h-11 flex flex-col items-center justify-center rounded-sm border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2.5 text-sm bg-gray-100 rounded-sm border border-gray-200 focus:border-orange-400 focus:outline-none transition-colors"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 rounded-sm hover:bg-gray-200 transition-colors"
            >
        ...
        <button
          ...
          className={`shrink-0 flex items-center justify-center w-10 h-10 rounded-sm border transition-colors ${
      ...
      {(selectedArea || selectedShop) && (
        <div className="max-w-md md:max-w-none mx-auto px-4 pb-2 flex items-center gap-2">
          {selectedArea && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-medium bg-orange-50 text-orange-600 rounded-sm">
              {selectedArea}
          ...
          {selectedShop && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 rounded-sm">
              {selectedShop}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-100 flex items-end sm:items-center justify-center">
          ...
          {/* モーダル本体 */}
          <div className="relative w-full max-w-md bg-white rounded-sm p-5 pb-8 sm:pb-5 animate-in slide-in-from-bottom duration-200">
            {/* ヘッダー */}
            ...
            <div className="mb-4">
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-sm border transition-colors ${
                  isCalendarOpen
              ...
              {isCalendarOpen && (
                <div className="mt-3 p-3 bg-gray-50 rounded-sm">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <button onClick={prevMonth} className="p-1 rounded-sm hover:bg-gray-200 transition-colors">
                      <ChevronLeft className="w-4 h-4 text-gray-500" />
                    </button>
                    <span className="text-sm font-bold text-gray-700">{calendarMonthLabel}</span>
                    <button onClick={nextMonth} className="p-1 rounded-sm hover:bg-gray-200 transition-colors">
                    ...
                    >
                      {shop.image_url && (
                        <Image
                          ...
                          width={18}
                          height={18}
                          className="rounded-sm object-cover w-4.5 h-4.5"
                          unoptimized
                        />
                      )}
                      {shop.name}
            ...
            <div className="flex gap-3">
              <button
                onClick={clearFilters}
                className="flex-1 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-sm hover:bg-gray-200 transition-colors"
              >
                リセット
              </button>
              <button
                onClick={applyAndClose}
                className="flex-1 py-2.5 text-sm font-bold text-white bg-orange-500 rounded-sm hover:bg-orange-600 transition-colors"
              >
