...
export function DigitalMemberCard({ 
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto aspect-[1.586/1] rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 hover:scale-[1.02]">
      ...
      <div
        className={cn(
          "absolute inset-0 bg-gradient-to-br animate-gradient-xy",
          isVip
      ...
      <div className="relative h-full flex flex-col justify-between p-6 text-white">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {isVip && <Crown className="w-5 h-5 text-yellow-200 fill-yellow-200" />}
              <span className={cn("text-2xl font-bold tracking-tight", isVip ? "text-yellow-100" : "text-white")}>
                {isVip ? "VIP PASS" : "MEMBER"}
              </span>
            </div>
          </div>
          {/* ロゴ的なもの */}
        ...
        <div className="flex justify-between items-end border-t border-white/20 pt-4">
          <div>
            <p className="text-[10px] opacity-70">CURRENT TIME</p>
