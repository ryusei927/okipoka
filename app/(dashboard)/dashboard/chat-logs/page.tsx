...
export default async function ChatLogsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-gray-900">AIチャットログ</h1>
            ...
            <tbody className="divide-y divide-gray-100">
              {logs?.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50 border-b border-gray-200">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-gray-500">
                    {log.user_id ? "会員" : "ゲスト"}
                    <div className="text-[10px] text-gray-500 font-mono">{log.session_id?.substring(0, 6)}</div>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-bold ${
                      log.role === 'user' ? 'bg-blue-50 text-blue-700' : 'bg-orange-50 text-orange-700'
                    }`}>
                      {log.role === 'user' ? 'User' : 'AI'}
                    </span>
                  </td>
                  <td className="px-4 py-3 max-w-xl break-words text-gray-500">
                    {log.content}
                  </td>
                </tr>
              ))}
